  ...
  <section class="w-full h-full px-4 sm:px-6 lg:px-12 py-8 overflow-y-auto">
    <div class="max-w-7xl mx-auto">
      ...
      <header class="mb-8">
        <div class="flex items-center gap-3 mb-3">
          <div class="h-1 w-12 bg-gradient-to-r from-yellow-500 to-orange-500 rounded-full"></div>
          <p class="text-sm uppercase tracking-[0.3em] text-yellow-400 font-semibold">
            Boutique
          </p>
        </div>
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div class="flex-1">
            <h2 class="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-slate-50 mb-3 bg-gradient-to-r from-slate-50 via-slate-100 to-slate-200 bg-clip-text text-transparent">
              🛒 Boutique
            </h2>
            <p class="text-sm sm:text-base text-slate-300 max-w-2xl">
              Achetez des items exclusifs avec vos coins. Personnalisez votre expérience de jeu !
            </p>
          </div>
          <!-- Affichage des coins -->
          <div v-if="userStore.isLoggedIn" class="flex items-center gap-2 sm:gap-3 px-4 sm:px-6 py-2.5 sm:py-3 rounded-xl sm:rounded-2xl bg-gradient-to-br from-yellow-500/20 to-orange-500/20 border border-yellow-500/30 backdrop-blur-sm flex-shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 sm:h-8 sm:w-8 text-yellow-400" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div>
              <p class="text-[10px] sm:text-xs text-slate-400">Vos coins</p>
              <p class="text-xl sm:text-2xl font-bold text-yellow-400">{{ userStore.coins }}</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        <div
          ...
          class="group relative rounded-2xl bg-gradient-to-br from-slate-900/90 to-slate-800/90 border border-slate-700/50 overflow-hidden shadow-xl hover:shadow-yellow-500/20 hover:border-yellow-500/30 transition-all duration-300 hover:scale-[1.02] cursor-pointer"
        >
          <!-- Image/Icone de l'item -->
          <div class="relative h-48 bg-gradient-to-br from-yellow-600/20 via-slate-900 to-slate-800 flex items-center justify-center overflow-hidden">
            <div class="text-8xl opacity-40 group-hover:scale-110 transition-transform duration-500">{{ item.icon }}</div>
          ...
          <div class="p-5">
            ...
            <div class="flex items-center justify-between mb-4">
              <div class="flex items-center gap-2">
                ...
                <span class="text-lg font-bold text-yellow-400">{{ item.price }}</span>
              </div>
            ...
            <button
              ...
              class="w-full px-4 py-3 rounded-xl bg-gradient-to-r from-yellow-500 to-orange-500 text-slate-900 text-sm font-bold tracking-wide hover:from-yellow-400 hover:to-orange-400 transition-all duration-300 shadow-lg shadow-yellow-500/30 hover:shadow-xl hover:shadow-yellow-500/40 hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 flex items-center justify-center gap-2"
            >
    ...
    >
      <div class="relative w-full max-w-2xl rounded-3xl bg-gradient-to-br from-slate-900 to-slate-800 border border-slate-700/50 shadow-2xl overflow-hidden">
        ...
        <div class="relative h-64 bg-gradient-to-br from-yellow-600/30 via-slate-900 to-slate-800 flex items-center justify-center overflow-hidden">
          <div class="text-9xl opacity-30">{{ selectedItem.icon }}</div>
          <button
          ...
          <div v-if="selectedItem.isOwned" class="absolute top-4 left-4">
            <span class="px-4 py-2 rounded-full text-sm font-bold bg-gradient-to-r from-green-500 to-emerald-500 text-white shadow-lg">
              ✓ Possédé
            </span>
          </div>
...
async function handlePurchase(item: Item) {
  if (!userStore.userId) {
    error.value = 'Vous devez être connecté pour acheter'
    return
  }

  if (userStore.coins < item.price) {
    error.value = 'Vous n\'avez pas assez de coins'
    return
  }
